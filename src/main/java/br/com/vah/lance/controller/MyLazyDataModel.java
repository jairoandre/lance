...
public class MyLazyDataModel<T extends BaseEntity> extends LazyDataModel<T> implements Serializable{
     ...
     * @param crudService
     */
    public MyLazyDataModel(DataAccessService crudService) {
    ...
    public List<T> load(int first, int pageSize, String sortField, SortOrder sortOrder,
    	...
    	datasource = crudService.findWithNamedQuery(type.getSimpleName() + "populatedItems", first, first + pageSize);
    	if(sortField != null){
    		Collections.sort(datasource, new LazySorter<>(sortField, sortOrder));
    	}
    	setRowCount(crudService.countTotalRecord(type.getSimpleName() + "totalCount"));
    	return datasource;
    }
